...
// 兼容outerHTML
// outHTML类似innerHTML，但是它包括标签本身
// 对于<div id="a"><p>aaaaa</p></div>
// innerHTML: <p>aaaaa</p>
// outerHTML: <div id="a"><p>aaaaa</p></div>

// el.cloneNode(deep): 拷贝el节点，包括所有的attributes
// 若deep为true，同时拷贝其子节点（即attributes和values）
// 否则，只拷贝节点本身和它的attributes

export function getOuterHTML (el) {
  // 如果浏览器支持outerHTML属性，直接使用
  if (el.outerHTML) {
    return el.outerHTML
  } else {
    // 如果不支持原生outerHTML
    // 首先创建一个名为container的div标签，拷贝el元素放入其中
    var container = document.createElement('div')
    container.appendChild(el.cloneNode(true))
    // 然后获取container的innerHTML
